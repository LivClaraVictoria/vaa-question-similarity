"""
CLI entry point for the clone generation pipeline. Reads a clone config, selects questions,
generates paraphrases if needed, applies clone specs, and writes the dataset to disk.
Called by the `generate-clones` subcommand of main.py.
"""
...
def main(argv=None):
    ...
    Usage:
        python -m main generate-clones --config configs/create_clones/identical_q32214_n10.py
    """
    ...
    needs_paraphrases = any(spec.clone_type != "identical" for spec in specs)
    paraphrases = None
    ...
    # 4. Apply
    modified_dfs = apply_specs(specs, dataframes, paraphrases=paraphrases)
