#!/bin/bash
# Launcher: Alpha sweeps for E5-instruct model vs all Q32214 clone types.
# Assumes clone datasets already exist (run launch_create_clones_q32214.sh first).
# Run with: bash jobs/launch_alpha_sweeps_q32214_e5_instruct.sh
...
echo "=== Alpha Sweeps: E5-instruct vs Q32214 clones ==="
echo ""

for CLONE in "${CLONE_TYPES[@]}"; do
    TIMESTAMP=$(date +%Y%m%d_%H%M%S)
    export CONFIG_B="configs/full_pipeline/cloned/${CLONE}_e5_instruct_ZH.py"
    SUBFOLDER="alpha_sweep_pipeline_e5_instruct_ZH_vs_${CLONE}_e5_instruct_ZH"
    export SWEEP_DIR="${RESULTS_BASE}/${SUBFOLDER}/workers_${TIMESTAMP}"
    mkdir -p "${SWEEP_DIR}"

    SWEEP_JOB=$(sbatch --parsable --export=ALL --array=0-20 --job-name="asw_${CLONE}_e5i" "${SCRIPT_DIR}/job_alpha_sweep_worker.sh")
    COLLECT_JOB=$(sbatch --parsable --export=ALL --dependency=afterok:${SWEEP_JOB} --job-name="asc_${CLONE}_e5i" "${SCRIPT_DIR}/job_alpha_sweep_collect.sh")
    echo "  ${CLONE}: sweep=${SWEEP_JOB}, collect=${COLLECT_JOB}"
    sleep 1  # ensure unique timestamps
done

echo ""
echo "=== 7 alpha sweep batches submitted (E5-instruct) ==="
