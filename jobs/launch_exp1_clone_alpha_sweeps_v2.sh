#!/bin/bash
# Launcher: Experiment 1 v2 — Alpha sweeps for all embedding models x 3 clone conditions.
# Uses updated top-5 impact questions (32214, 32228, 32234, 32240, 32261) with 4 clones each.
# (v1 used 32214, 32261, 32228, 32234, 32222 — Q32222 replaced by Q32240 for 4-clone consistency.)
#
...
# (distinct from v1 results in top5impact/)
#
...
export CLONE_CONFIG="configs/create_clones/easy_paraphrase_top5impact_v2_n4.py"
CLONE1=$(sbatch --parsable --export=ALL --job-name=clone_ep_v2 "${SCRIPT_DIR}/job_create_clone_single.sh")
echo "  easy_paraphrase_top5impact_v2_n4:  job ${CLONE1}"

export CLONE_CONFIG="configs/create_clones/negation_easy_top5impact_v2_n4.py"
CLONE2=$(sbatch --parsable --export=ALL --dependency=afterok:${CLONE1} --job-name=clone_ne_v2 "${SCRIPT_DIR}/job_create_clone_single.sh")
echo "  negation_easy_top5impact_v2_n4:    job ${CLONE2} (after ${CLONE1})"

export CLONE_CONFIG="configs/create_clones/mixed_top5impact_v2_n4.py"
CLONE3=$(sbatch --parsable --export=ALL --dependency=afterok:${CLONE2} --job-name=clone_mix_v2 "${SCRIPT_DIR}/job_create_clone_single.sh")
echo "  mixed_top5impact_v2_n4:            job ${CLONE3} (after ${CLONE2})"
...
CLONE_CONDITIONS=("easy_paraphrase_top5impact_v2_n4" "negation_easy_top5impact_v2_n4" "mixed_top5impact_v2_n4")
...
echo ""
echo "=========================================="
echo "  Experiment 1 v2 submitted!"
