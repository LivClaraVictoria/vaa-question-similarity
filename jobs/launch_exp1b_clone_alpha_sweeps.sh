#!/bin/bash
# Launcher: Experiment 1b — Alpha sweeps for 3 NEW clone conditions x 10 embedding models.
# Clone conditions: hard_paraphrase, negation_hard, perfect_mix (1 of each type).
...
# Run with: bash jobs/launch_exp1b_clone_alpha_sweeps.sh
...
echo "=========================================="
echo "  Experiment 1b: Hard Clone Alpha Sweeps"
...
export CLONE_CONFIG="configs/create_clones/hard_paraphrase_top5impact_n4.py"
CLONE1=$(sbatch --parsable --export=ALL --job-name=clone_hp_top5 "${SCRIPT_DIR}/job_create_clone_single.sh")
echo "  hard_paraphrase_top5impact_n4:  job ${CLONE1}"

export CLONE_CONFIG="configs/create_clones/negation_hard_top5impact_n4.py"
CLONE2=$(sbatch --parsable --export=ALL --dependency=afterok:${CLONE1} --job-name=clone_nh_top5 "${SCRIPT_DIR}/job_create_clone_single.sh")
echo "  negation_hard_top5impact_n4:    job ${CLONE2} (after ${CLONE1})"

export CLONE_CONFIG="configs/create_clones/perfect_mix_top5impact_n4.py"
CLONE3=$(sbatch --parsable --export=ALL --dependency=afterok:${CLONE2} --job-name=clone_pm_top5 "${SCRIPT_DIR}/job_create_clone_single.sh")
echo "  perfect_mix_top5impact_n4:      job ${CLONE3} (after ${CLONE2})"
...
CLONE_CONDITIONS=("hard_paraphrase_top5impact_n4" "negation_hard_top5impact_n4" "perfect_mix_top5impact_n4")
